...
async function main() {
  ...
  const mockSemaphoreFactory = await hre.ethers.getContractFactory("MockSemaphore");
  ...
  const gateFactory = await hre.ethers.getContractFactory("ComplianceGate");
  ...
  // 3. Deploy AccessNFT
  const nftFactory = await hre.ethers.getContractFactory("AccessNFT");
